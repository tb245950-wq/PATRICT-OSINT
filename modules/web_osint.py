...
class WebOSINT(BaseOSINTModule):
        ...
        try:
                    ...
                    if cert:
                        ...
                        if not_after:
                            try:
                                ...
                                cert_info["days_remaining"] = max(0, diff)
            ...
            try:
                ...
                with socket.create_connection((domain, 443), timeout=5.0) as sock:
                    with ctx_fallback.wrap_socket(sock, server_hostname=domain) as ssock:
                        cert_info["has_ssl"] = True
                        cert_info["tls_version"] = ssock.version() or "TLS"
                        c = ssock.cipher()
    ...
    async def _fetch_crtsh_subdomains(self, domain: str) -> Dict[str, Any]:
        ...
        Menemukan seluruh subdomain tanpa mengirim request mencurigakan ke target.
        """
        ct_data = {
            "total_found": 0,
            "unique_subdomains": [],
        ...
        try:
            ...
            status, text, _ = await self.async_client.get(url, timeout=10)
            ...
            self.logger.debug(f"Gagal mengambil CT logs dari crt.sh: {e}")
    ...
    async def _probe_sensitive_files(self, base_url: str) -> List[Dict[str, Any]]:
        ...
        for path, desc, severity in candidates:
            ...
            try:
                ...
                if status in (200, 301, 302, 401, 403):
                    ...
                    # False-positive filtering untuk custom 404 handler yang mengembalikan 200 OK
                    if status == 200:
                        ...
                        if path in ("/.env", "/.env.local", "/.env.production", "/config.json", "/web.config", "/storage/logs/laravel.log", "/backup.sql", "/db.sql", "/Dockerfile", "/docker-compose.yml"):
                            ...
                            if not ("ref: refs/" in body or len(body.strip()) == 40 or "ref:" in body_lower):
                                is_real = False
                        elif path == "/phpinfo.php" or path == "/info.php":
    ...
    def _generate_threat_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if ssl_data.get("is_expired"):
            ...
            threats.append({
                "category": "SSL/TLS Security",
                "severity": "MEDIUM",
                "title": f"Sertifikat SSL/TLS Akan Kedaluwarsa dalam {ssl_data.get('days_remaining')} hari",
        ...
        if not flags.get("httponly") or not flags.get("secure"):
            risk_score += 10
            threats.append({
